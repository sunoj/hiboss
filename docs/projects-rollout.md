...
before deploying this Worker, then upgrade CLIs. This change does not deploy itself.
...
Whitespace-only values are skipped. Slugs are lowercase ASCII letters, digits,
underscores and dashes; runs of punctuation become one dash. Distinct legacy values
that normalize to the same slug retain separate identities with deterministic
`--<UTF-8 hex>` suffixes. Empty normalized slugs use `project--<UTF-8 hex>`.
This avoids silently merging unrelated historical data.
...
An alias match wins over the requested slug. Otherwise the canonical slug can match
an existing project, or the server creates one. All supplied aliases, including the
raw slug, are registered atomically. Conflicting aliases return 409 rather than
merging or reassigning projects. Concurrent first sightings retry identity resolution;
...
alias; it does not rename an identity already found through another alias.
...
record of renamed checkouts, not a temporary migration shim. No project merge API
or automatic merging of preexisting identities is introduced in 3a.
...
The schema test runner includes four SQLite migration fixtures for profiles,
labels, posts, routes, slug collisions, message preservation, aliases and unique names.
CLI validation uses `cargo check -p hiboss` and `cargo test -p hiboss session::`,
