...
export function parseControl(value: unknown): ControlCommand {
  ...
  const keys = ['protocolVersion', 'action', 'expectedMetadataVersion', 'expectedDefinitionRevision', 'expectedEpoch', 'expectedState', 'openRequests', 'withdrawalReason', 'finalTask', 'result', 'dismissal'];
  ...
  if (!['reject', 'withdraw'].includes(String(value.openRequests))) throw new PanelFault('unsupported_request_policy', 422);
  if (value.openRequests === 'withdraw' && (typeof value.withdrawalReason !== 'string' || !value.withdrawalReason.trim() || value.withdrawalReason.length > 1000
    || !['complete', 'fail', 'cancel'].includes(String(value.action)))) throw new PanelFault('withdrawal_reason_required', 422);
  if (value.openRequests === 'reject' && value.withdrawalReason !== undefined) throw new PanelFault('invalid_command', 422);
...
export async function prepareControl(row: PanelRecord, snapshot: Checkpoint, agentId: string, key: string, command: ControlCommand, epoch: string | null): Promise<PendingControl> {
  ...
  return { ...(command.openRequests === 'withdraw' ? { withdrawalReason: command.withdrawalReason } : {}), operationId: crypto.randomUUID(), panelId: row.panel_id, agentId, key, hash: await bodyHash(command), expectedVersion: row.metadata_version,
...
export async function commitControl(db: D1Database, pending: PendingControl): Promise<unknown> {
  ...
  const result = await db.batch([
    ...
    ...(pending.withdrawalReason ? [db.prepare(`UPDATE interaction_requests SET state = 'withdrawn', withdrawal_reason = ? WHERE panel_id = ? AND state = 'open' AND EXISTS (${guard})`)
      .bind(pending.withdrawalReason, pending.panelId, pending.panelId, pending.operationId)] : []),
  ...
  if (result[0].meta.changes !== 1) {
    const open = terminal(pending.lifecycle.taskState) && !pending.withdrawalReason && await db.prepare("SELECT 1 FROM interaction_requests WHERE panel_id = ? AND state = 'open' AND (expires_at IS NULL OR julianday(expires_at) > julianday('now'))").bind(pending.panelId).first();
    if (open) throw new PanelFault('open_requests', 409, 'Answer or explicitly withdraw the open questionnaires before ending this panel');
    throw new PanelFault('revision_conflict');
  }
  return receipt;
}
