...
async function drainDeliveryQueue(env: Env, now: string): Promise<void> {
  const rows = await env.DB
    .prepare(
      `SELECT id, message_id, agent_id, channel, config, scheduled_at
       FROM delivery_queue
  ...
  for (const row of rows.results ?? []) {
    const locked = await markQueueProcessing(env, row.id);
    if (!locked) continue;
    try {
      ...
      await markQueueFailed(env, row.id, message);
    }
  }
}
...
async function markQueueProcessing(env: Env, queueId: string): Promise<boolean> {
  const result = await env.DB
    .prepare("UPDATE delivery_queue SET status = 'processing', attempts = attempts + 1, error = NULL WHERE id = ? AND status = 'pending'")
    .bind(queueId)
    .run();
...
async function markQueueFailed(env: Env, queueId: string, error: string): Promise<void> {
  await env.DB
    .prepare("UPDATE delivery_queue SET status = 'failed', error = ? WHERE id = ?")
    .bind(error, queueId)
    .run();
}
