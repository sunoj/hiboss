...
import { defaultLifecycle, defaultPreference, PanelFault, type Lifecycle, type Preference, type Checkpoint, type PanelId } from './types';

export interface PanelRecord {
  panel_id: PanelId; agent_id: string; target_boss_id: string; definition_revision: number; metadata_version: number;
  lifecycle_json: string; final_snapshot_json: string | null; initial_state_json: string; state_schema_json: string;
}
...
export function initialCheckpoint(row: PanelRecord): Checkpoint {
  ...
  return { protocolVersion: 2, serverTime: Date.now(), panelId: row.panel_id, definitionRevision: row.definition_revision, epoch: null, sequence: 0,
    task: root.task, observationVersion: 0, lastObservedAt: null, staleAt: null, leaseExpiresAt: null, persistedAt: 0 };
}
...
export function publicationLifecycle(value: unknown): Lifecycle {
  ...
  if (!isRecord(value) || Object.keys(value).some(k => !['mode', 'expectedUpdateIntervalSeconds'].includes(k))) throw new PanelFault('invalid_lifecycle', 422);
  const mode = value.mode ?? 'run';
  const interval = value.expectedUpdateIntervalSeconds ?? 15;
  if (!['run', 'monitor'].includes(String(mode)) || typeof interval !== 'number' || !Number.isInteger(interval) || interval < 5 || interval > 3600) throw new PanelFault('invalid_lifecycle', 422);
  return { ...defaultLifecycle(), mode: mode as Lifecycle['mode'], expectedUpdateIntervalSeconds: interval };
}
