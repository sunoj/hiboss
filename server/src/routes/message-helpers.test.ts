...
describe('buildFilters', () => {
  ...
  it('adds direction filter', () => {
    ...
    expect(where).toBe('(agent_id = ? OR target_agent_id = ?) AND direction = ?');
    expect(binds).toEqual(['agent-1', 'agent-1', 'agent_to_boss']);
  });

  it('adds status filter', () => {
    const { where, binds } = buildFilters('agent-1', null, 'sent');
    expect(where).toBe('(agent_id = ? OR target_agent_id = ?) AND status = ?');
    expect(binds).toEqual(['agent-1', 'agent-1', 'sent']);
  });

  it('adds priority IN filter', () => {
    const { where, binds } = buildFilters('agent-1', null, null, ['critical', 'high']);
    expect(where).toBe('(agent_id = ? OR target_agent_id = ?) AND priority IN (?, ?)');
    expect(binds).toEqual(['agent-1', 'agent-1', 'critical', 'high']);
  });

  it('combines all filters', () => {
    const { where, binds } = buildFilters('a1', 'boss_to_agent', 'read', ['normal']);
    expect(where).toBe('(agent_id = ? OR target_agent_id = ?) AND direction = ? AND status = ? AND priority IN (?)');
    expect(binds).toEqual(['a1', 'a1', 'boss_to_agent', 'read', 'normal']);
  });
