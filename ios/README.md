...
The QR scanner accepts `hiboss://pair` links generated by the macOS client. A link
contains the server URL and a five-minute, single-use pairing code; it does not
contain a Boss Token. The app fills the scanned server URL before redemption,
registers its Secure Enclave signing key, and stores the resulting device token in
Keychain. Restoration requires both a valid stored URL and token, so an orphan token
cannot skip onboarding or appear as a valid login.
