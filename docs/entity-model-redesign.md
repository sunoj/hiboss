...
| `progress_posts` | **phase 3b: implemented** — FK-only reads/writes, API requires resolution; old CLI text request/response adapter retained. Boss posts expose project objects. Text column retained nullable until a later phase | 0044 preserves posts and likes while removing the text NOT NULL constraint |
