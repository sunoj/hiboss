...
pub async fn run(args: &InitArgs, config: &mut Config) -> Result<(), Box<dyn Error>> {
    ...
    if join.status == "approved" {
        let key = join
            .key
            .ok_or("Server approved the join request but did not return a key.")?;
    ...
    let request_id = join
        .request_id
        .ok_or("Server returned a pending join without a request ID.")?;
    let poll_token = join
        .poll_token
        .ok_or("Server returned a pending join without a poll token.")?;
    eprintln!(
        "{} Waiting for boss approval... (request: {})",
        "⏳".yellow(),
        shorten_id(&request_id)
    );
...
async fn poll_for_approval(
    ...
    loop {
        ...
        if start.elapsed() > timeout {
            eprintln!(
                "{} Timed out waiting for approval. Ask your boss to approve, then run `hiboss init {server}` again.",
                "⏱".red()
            );
            return Err("Approval timed out.".into());
        }
        match fetch_join_status(client, &poll_url).await? {
            Some(status) if status.status == "approved" => {
                return finish_approval(config, server, name, status);
            }
            Some(status) if status.status == "rejected" => {
                return Err("Join request rejected by boss.".into());
            }
...
async fn fetch_join_status(
    client: &Client,
    poll_url: &str,
) -> Result<Option<JoinStatusResponse>, Box<dyn Error>> {
